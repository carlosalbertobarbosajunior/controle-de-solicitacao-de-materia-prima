SELECT
	...
	ISNULL(TORDC.ENVIADO_FORNECEDOR, 0) AS 'Enviado ao Fornecedor',
	TCOMP.CODIGO AS 'Cód. Entrada',
	TCOMP.N_NF AS 'NF',
	TLM.COD_INTERNO AS 'Cód. Interno do Produto',
	TPROD.FAMILIA AS 'Família',
	TLM.PRODUTO AS 'Produto',
	TORDC.FORNECEDOR AS 'Fornecedor',
	TORC.CLIENTE AS 'Cliente',
	CONVERT(DATE, TORCI.DT_PREVISAO_ENTREGA) AS 'Data de Conclusão do Contrato',
	CONVERT(DATE, TSOL.DT_SOLICITACAO) AS 'Data de Solicitação',
	CONVERT(DATE, TSOL.DT_NECESSIDADE) AS 'Data de Necessidade',
	CONVERT(DATE, TORDC.DATA) AS 'Data da OC',
	CONVERT(DATE, TSOL.DT_PREVISTA_ENTREGA) AS 'Data Prevista de Entrega',
	CONVERT(DATE, TCOMP.DT_RECEBIMENTO) AS 'Data de Entrega',
	CASE
		WHEN (DATEDIFF(DAY,CONVERT(DATE, TORDC.DATA),CONVERT(DATE, TCOMP.DT_RECEBIMENTO))) < 0 THEN 0
		ELSE (DATEDIFF(DAY,CONVERT(DATE, TORDC.DATA),CONVERT(DATE, TCOMP.DT_RECEBIMENTO)))
	END AS 'LT OC-Entrega (Dias)',
...
FROM
	(SELECT * FROM TLIS_MAT WHERE COD_EMPRESA = 1 AND DT_NECESSIDADE > '2022-06-01') AS TLM

--Materiais
INNER JOIN (SELECT * FROM TPRODUTO WHERE COD_EMPRESA = 1) AS TPROD
	ON TLM.COD_INTERNO = TPROD.CODIGO_INTERNO

--Serviços e Orçamentos
INNER JOIN (SELECT COD_OS, CODIGO, COD_OS_COMPLETO, SUBTITULO FROM TOS_AUX WHERE COD_OS_COMPLETO NOT LIKE 'E%') AS TOSA
	ON TLM.COD_OS = TOSA.COD_OS AND TLM.COD_OS_AUX = TOSA.CODIGO /*AND TLM.COD_OS_COMPLETO = TOSA.COD_OS_COMPLETO*/
		INNER JOIN (SELECT CODIGO, GUID_ORCAMENTO, N_OS FROM TOS WHERE N_OS <> 1 AND APROVADO = 1 AND CANCELADO = 0 AND N_OS NOT LIKE 'E%' AND CONCLUIDO = 0 AND YEAR(DT_ENTRADA) >= 2021) AS TOS
			ON TOSA.COD_OS = TOS.CODIGO
				INNER JOIN (SELECT GUID_LINHA, COD_ORCAMENTO, DT_PREVISAO_ENTREGA FROM TORCAMENTO_ITENS) AS TORCI
	...
	ON TLM.GUID_LINHA = TORDOS.GUID_LM
		LEFT JOIN (SELECT GUID_LINHA, COD_ORD_COMPRA, PLANO_CONTAS FROM TORD_AUX) AS TORD
			ON TORDOS.GUID_PAI = TORD.GUID_LINHA
				LEFT JOIN (SELECT CODIGO, DATA, ENVIADO_FORNECEDOR, FORNECEDOR FROM TORD_COM) AS TORDC
